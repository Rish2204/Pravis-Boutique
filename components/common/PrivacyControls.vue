<template>
  <div class="privacy-controls">
    ...
    <div v-if="showConfirmDialog" class="confirmation-dialog">
      <div class="dialog-content">
        ...
        <div class="dialog-actions">
          <button class="privacy-button cancel-button" @click="showConfirmDialog = false">Cancel</button>
          <button class="privacy-button delete-button" @click="deleteUserData">Yes, Delete My Data</button>
        </div>
      </div>
    </div>
    ...
    <div class="privacy-summary">
      ...
      <p>We use this information to improve our services and personalize your shopping experience.</p>
...
const hasConsent = computed(() => {
  ...
  try {
    const stored = localStorage.getItem('pravis-consent')
    if (!stored) return false
    return JSON.parse(stored).consent === true
  } catch {
    return false
  }
})
...
const toggleConsent = () => {
  const current = hasConsent.value
...
const deleteUserData = () => {
  ...
  const keys = Object.keys(localStorage).filter(k => k.startsWith('pravis'))
...
.consent-status { margin-bottom: 24px; }
...
input:checked + .toggle-slider { background-color: #8B0000; }
...
.delete-button:hover { background-color: #c0392b; transform: translateY(-2px); }
.cancel-button { background-color: #95a5a6; color: white; }
...
.dialog-content h4 { margin-top: 0; color: #e74c3c; font-size: 20px; }
...
.privacy-summary h4 { color: #2c3e50; margin-bottom: 8px; font-size: 18px; }
.privacy-summary ul { padding-left: 20px; margin-bottom: 24px; color: #555; }
.privacy-summary p { color: #555; line-height: 1.6; margin-bottom: 24px; }
.privacy-links { display: flex; gap: 24px; margin-top: 16px; }
.privacy-links a { color: #8B0000; text-decoration: none; font-weight: 500; }
